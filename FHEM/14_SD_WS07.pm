##############################################
# $Id: 14_SD_WS07.pm 351 2020-12-22 21:14:53Z Sidey $
#
...
# 2018 - 2019  Sidey79, elektron-bbs
# 2020         HomeAuto_User
#
...
sub SD_WS07_Parse($$) {
  ...
  if (AttrVal($name, 'negation-batt', 'no') eq 'yes') { # default undef negation batt bit
    ...
    readingsBulkUpdate($hash, 'temperature', $temp)  if ($temp ne q{});
    ...
    readingsBulkUpdate($hash, 'channel', $channel) if ($channel ne q{});
...
<ul>
  ...
  <ul>
     ...
     <li>channel: (the channelnumber)</li>
